...
import { ElCard, ElTag, ElAvatar } from 'element-plus';
    ...
    <div class="meeting-detail-container">
      ...
      <ElCard class="info-card">
        <template #header>
          <div class="card-header">
            ...
            <ElTag :type="meetingStatusTagType" size="large">{{ meetingStatusText }}</ElTag>
          </div>
        </template>
        
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <InfoItem label="会议类型" :value="meetingTypeText" icon="calendar" />
          <InfoItem label="会议地点" :value="meetingPlaceText" icon="location" />
          <InfoItem label="会议时间" :value="formatMeetingTime" icon="clock" />
          <InfoItem label="记录人" :value="meetingInfo.recorder || '未指定'" icon="user" />
        </div>
      </ElCard>
      ...
      <ElCard class="info-card">
        <template #header>
          <div class="card-header">
            <h3>与会人员</h3>
            <span class="participant-count">{{ meetingInfo.participantsCount || 0 }}人</span>
          </div>
        </template>
        ...
        <div class="resolutions-container">
          <div v-for="(resolution, index) in resolutions" :key="index" class="resolution-item">
            <div class="resolution-header">
              ...
              <ElTag v-if="resolution.status === 'completed'" type="success">已完成</ElTag>
              <ElTag v-else-if="resolution.status === 'in-progress'" type="warning">进行中</ElTag>
              <ElTag v-else type="info">未开始</ElTag>
            </div>
            <div class="resolution-content">{{ resolution.content }}</div>
            <div class="resolution-footer">
              <div><span class="label">负责人：</span>{{ resolution.owner }}</div>
              <div><span class="label">截止日期：</span>{{ resolution.deadline }}</div>
            </div>
          </div>
        </div>
      </ElCard>
